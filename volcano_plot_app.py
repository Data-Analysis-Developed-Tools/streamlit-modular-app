...
def mostra_volcano_plot():
    ...
    try:
        ...
        fig.update_traces(
            ...
            "<span style='font-size:10px'>Valore medio (sovraespresso): %{customdata[0]}</span><br>" +
            "<span style='font-size:10px'>Valore medio (sottoespresso): %{customdata[1]}</span><extra></extra>",
            text=dati_preparati['Variabile'],
            customdata=np.stack([
                dati_preparati["MediaClasseSopra"],
                dati_preparati["MediaClasseSotto"]
            ], axis=-1)
        )
